...
for i in range(pre_num):
    ...
    pre_2 = gm_forecast2['h.1']
    #print( pre_2[0])

    if pre_2[0] > np.max(vol2_retn_fill[:window_size+i]):
        pre_2[0] = np.max(vol2_retn_fill[:window_size+i])
    if pre_2[0] < np.min(vol2_retn_fill[:window_size+i]):
        pre_2[0]= np.min(vol2_retn_fill[:window_size+i])
    if pre_2[0] > vol2_retn_fill[:window_size+i][-1] * 4:
        pre_2[0] =np.mean(vol2_retn_fill[:window_size+i])
        # pre_2[0]= vol2_retn_fill[:window_size+i][-1] * 4
    pre_u_d= pre_2[0]-vol2_retn_fill[:window_size+i][-1]
    pre_u_d_s=np.append( pre_u_d_s,pre_u_d)
    gm_pre_2 = np.append( gm_pre_2,pre_2[0] )
